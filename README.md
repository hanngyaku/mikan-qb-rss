...
Mikan RSS 中的 `bangumiId` 会关联到对应的 `/Home/Bangumi/{id}` 页面，缩略图下载到 `data/posters/{id}.webp`；放送日期、放送开始、官方网站、Bangumi 链接和概况介绍保存在 SQLite，并显示在订阅详情。已有订阅点击“同步”即可补齐元数据。

默认首页为按星期排列的放送表。缺少放送日期的订阅显示在底部“未知”区域，可拖动到星期列并保存人工放送日；人工设置不会被后续元数据同步覆盖。
